...
EOF
    ...
    cp "${FREEDOS_CACHE_DIR}/OS8AUTO.BAT" "${FREEDOS_CACHE_DIR}/FDAUTO.BAT"
    cp "${FREEDOS_CACHE_DIR}/OS8AUTO.BAT" "${FREEDOS_CACHE_DIR}/AUTOEXEC.BAT"

    cat > "${FREEDOS_CACHE_DIR}/OS8CONF.SYS" <<'EOF'
LASTDRIVE=Z
EOF

    cp "${FREEDOS_CACHE_DIR}/OS8CONF.SYS" "${FREEDOS_CACHE_DIR}/FDCONFIG.SYS"
    cp "${FREEDOS_CACHE_DIR}/OS8CONF.SYS" "${FREEDOS_CACHE_DIR}/CONFIG.SYS"
    ...
    mdel -i "$FREEDOS_BOOT_IMAGE" ::/OS8CONF.SYS >/dev/null 2>&1 || true
    ...
    mcopy -o -i "$FREEDOS_BOOT_IMAGE" "${FREEDOS_CACHE_DIR}/OS8CONF.SYS" ::/OS8CONF.SYS
