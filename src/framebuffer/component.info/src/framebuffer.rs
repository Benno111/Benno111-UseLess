...
pub fn set_text_opacity(opacity: u8) {
    TEXT_OPACITY.store(opacity as usize, Ordering::Relaxed);
}
...
/// Draw the desktop background (solid fill with grid overlay).
pub fn draw_desktop() {
    let _ = with_fb(|buf, info| {
        // Solid base.
        ...
        for y in 0..info.height {
            ...
            for x in 0..info.width {
                let bpp = info.bytes_per_pixel;
                let idx = row_off + x * bpp;
                if let Some(px) = buf.get_mut(idx..idx + bpp) {
                    set_pixel_bytes(px, bpp, 18, 20, 26, info);
                }
            }
        }

        // Grid overlay.
        let grid_color = (28u8, 31u8, 38u8);
        let step = 40usize;
        let bpp = info.bytes_per_pixel;
        for y in (0..info.height).step_by(step) {
            ...
            for x in 0..info.width {
                let idx = row_off + x * bpp;
                if let Some(px) = buf.get_mut(idx..idx + bpp) {
                    set_pixel_bytes(px, bpp, grid_color.0, grid_color.1, grid_color.2, info);
                }
            }
        }
        for x in (0..info.width).step_by(step) {
            for y in 0..info.height {
                let idx = y * stride_bytes + x * bpp;
                if let Some(px) = buf.get_mut(idx..idx + bpp) {
                    set_pixel_bytes(px, bpp, grid_color.0, grid_color.1, grid_color.2, info);
                }
            }
...
fn draw_glyph(buf: &mut [u8], info: &FrameBufferInfo, x: usize, y: usize, ch: u8, cfg: &FontConfig) {
    ...
    let alpha = TEXT_OPACITY.load(Ordering::Relaxed).min(255) as u8;
    let blend = alpha < 255;
            ...
            if row_bits & (1 << (4 - col)) != 0 {
                for sy in 0..scale {
                    for sx in 0..scale {
                        ...
                        let py = row_top + sy;
                        if blend {
                            blend_pixel(buf, info, px, py, cfg.fg, alpha);
                        } else {
                            draw_pixel(buf, info, px, py, cfg.fg);
                        }
                    }
                }
                ...
                for sy in 0..scale {
                    for sx in 0..scale {
                        ...
                        let py = row_top + sy;
                        if blend {
                            blend_pixel(buf, info, px, py, bg, alpha);
                        } else {
                            draw_pixel(buf, info, px, py, bg);
                        }
                    }
                }
            }
...
fn blend_pixel(
    ...
    if x >= info.width || y >= info.height || alpha == 0 {
        return;
    }
    let bpp = info.bytes_per_pixel;
    let idx = y * info.stride.saturating_mul(bpp) + x * bpp;
