...
use crate::{drivers, input, vga_buffer};
...
pub fn render(x: usize, y: usize, w: usize, h: usize) {
    ...
    if row_y + 12 < y + h {
        ...
        let poll_skips = drivers::poll_skip_count();
        let poll_budget = drivers::current_poll_budget();
        let log_contended = vga_buffer::log_contention_count();
        framebuffer::draw_text_no_invalidate(
            &format!("Input queue: {:>3}  Poll skips: {:>2}  Budget: {:>2}B", input_q, poll_skips, poll_budget),
            x + 2,
            detail_y,
        );
        if detail_y + 12 < y + h {
